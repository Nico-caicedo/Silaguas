<template>
  <q-layout container style="height: 92vh">
    ...
    <q-page-container>
      <q-scroll-area class="T-primary" style="height: calc(92vh - 100px)">
...
export default {
  ...
  data() {
    return {
      usuarioAcceso: {
        Login: '', Password: '', IdTercero: -1,
        NombreTercero: '', Nomodpass: false, Modpasspi: false, Passnoexpire: false, Activo: false, Fecha: utilidades.fechaActual()
      },
      ...
      usuario: {
        Login: '', IdRol: '', IdTercero: -1, Password: '',

        LoginCrea: '', Observacion: '', FIngreso: '', ModPasspi: '',

        NoModPass: 0, PassNoExpire: 0, Activo: 0, ProximoCambio: '',

        PeriodoCambio: 0, Estado: 0, RazonSocial: ''
      },
  ...
  methods: {
    ...
    limpiarTercero() {
      this.tercero = { Id: -1, Identificacion: '', Nombre: '', OtrosNombres: '', Apellido1: '', Apellido2: '', Alias: '', Direccion: '', Telefono: '', Celular: '', Email: '', Web: '', Observaciones: '', Login: '', Modifica: '' }
    ...
    filterFn(val, update, abort) {
      ...
      this.timeout = setTimeout(() => {
        api.get(`/tercero/mostrar-tercero-filtro-gral/${val}`)
          .then((response) => {
            update(() => {
    ...
    filterFnUsuario(val, update, abort) {
      ...
      clearTimeout(this.timeout)
      this.timeout = setTimeout(() => {
        api.get(`/usuario/usuarios-filtro/${val}`)
          .then((response) => {
            update(() => {
    ...
    mostrarusuarioPersona(persona) {
      ...
      self.$q.loading.show()
      api.get(`/usuario/usuario-id-tercero/${IdTercero}`)
        .then((response) => {
