    ...
    <q-page-container>
      <q-form @submit="guardarCambios">
        <div class="q-pa-md">
          <div class="row q-col-gutter-sm">
            <q-input class="col-xs-12 col-md-2 q-field-with-bottom" filled v-model="tercero.IdTercero" type="number"
              label="IdTercero" readonly />
            <q-input class="col-xs-12 col-md-4 q-field--with-bottom" filled v-model="tercero.Identificacion"
              @blur="cargarDatosIdentificacion" type="number" label="Identificacion" lazy-rules :rules="[
            (val) =>
              (val && val.length > 0) || 'You must enter an identification',
          ]" />

            <!-- <q-input class="col-xs-12 col-md-4 q-field--with-bottom" filled v-model="tercero.Identificacion" @input="(val) => {
            cargarDatosIdentificacion(val);
          }
            " :clearable="clearableTercero" @clear="limpiarTercero" type="number" label="Identificacion" lazy-rules
              :rules="[
            (val) => (val && val.length > 0) || 'ingresa identificacion',
          ]" /> -->
            <q-select class="col-xs-12 col-md-4 q-field--with-bottom" outlined map-options emit-value
              option-value="IdTipoIdentificacion" option-label="Nombre" :rules="[regla]"
              v-model="tercero.IdTipoIdentificacion" :options="tipoId" label="Tipo Identificación" />
            ...
            (val) => (val && val.length > 0) || 'Debe ingresar Apellido 1',
          ]" />
            ...
            <q-select class="col-xs-12 col-md-4 q-field--with-bottom" outlined filled :rules="[regla]"
              v-model="tercero.Sexo" map-options emit-value option-value="id" option-label="nombre" :options="sexo"
              label="Sexo">
              <template v-slot:prepend>
                <q-icon name="wc" size="lg" />
              </template>
            </q-select>
            <q-input class="col-xs-12 col-md-4 q-field--with-bottom" outlined stack-label :rules="[regla]"
              v-model="tercero.Nacimiento" type="date" label="Fecha de Nacimiento">
              <template v-slot:prepend>
            ...
            <q-select class="col-xs-12 col-md-6 q-field--with-bottom" outlined v-model="tercero.CodigoDepartamento"
              ...
              :options="listaDepartamentos" :rules="[regla]" @input="(val) => {
              ...
              <template v-slot:option="scope">
                <q-item v-bind="scope.itemProps" v-on="scope.itemEvents">
                  <q-item-section>
                    ...
                    <q-item-label caption>Código Departamento:
                      {{ scope.opt.CodigoDepartamento }}</q-item-label>
                  </q-item-section>
                </q-item>
              </template>
            ...
            <div class="row justify-center q-pa-sm">
              <q-btn label="Guardar" icon="save" type="submit" outline align="center" unelevated
      ...
      <q-dialog v-model="isDialogComponenteTerceros" style="max-width: 650px" :maximized="true">
        <q-card>
          ...
          <q-card-section>
            <component tercero="@tercero" :is="componenteTerceros"> </component>
          </q-card-section>
        </q-card>
      </q-dialog>
    </q-page-container>
...
export default {
  ...
  data() {
    return {
      ...
      ColumListaPrecios: [
        ...
        {
          name: "Nombre_listaprecio",
          align: "left",
          label: "Nombre",
          field: "Nombre_listaprecio",
          sortable: true,
        },
      ],
      terceroB: {
  ...
  methods: {
    ...
    cargarDatosIdentificacion() {
      const self = this;
      if (
        self.tercero.Identificacion === "" ||
        self.tercero.Identificacion === undefined
      ) {
        return;
      }
      ...
      self.$q.loading.show();
      api
        .get(`/tercero/mostrar-identificacion/${identificacion}`)
        .then((response) => {
        ...
        .catch((error) => {
          utilidades.mensaje(
            "Buscar Identificacion - Fallo la conexion " + error
          );
          self.$q.loading.hide();
        });
    },
    ...
    guardar() {
      ...
      api
        .post("/tercero/guardar/", this.tercero)
        .then((response) => {
          self.terceroB = response.data;
          if (self.terceroB.IdTercero > 0) {
    ...
    modificar() {
      ...
      self.$q.loading.show();
      api
        .post("/tercero/modificar/", this.tercero)
        .then((response) => {
          if (response.data > 0) {
    ...
    guardarCliente() {
      ...
      self.$q.loading.show();
      api
        .post("/tercero/guardarCliente/", this.cliente)
        .then((response) => {
    ...
    modificarCliente() {
      ...
      self.$q.loading.show();
      api
        .post("/tercero/modificarCliente/", this.cliente)
        .then((response) => {
          this.cliente = {};
          this.selected = [];
          self.$q.loading.hide();
    ...
    mostrarListaDepartamentos() {
      ...
      self.$q.loading.show();
      api
        .get("/usuario/mostrarDepartamentosMunicipios")
        .then((response) => {
    ...
    mostrarTipoIdentificacion(idTi, nombreTi) {
      ...
      self.$q.loading.show();
      api
        .get(`/tercero/tipoIdFiltro/${idTi}/${nombreTi}`)
        .then((response) => {
        ...
        .catch((error) => {
          utilidades.mensaje(
            "Tipo Identificacion - Fallo la conexion " + error
          );
          self.$q.loading.hide();
        });
    },
