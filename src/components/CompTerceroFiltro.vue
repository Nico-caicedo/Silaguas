        ...
        <div class="q-pa-md">
          <div class="row q-col-gutter-sm">
            <q-input class="col-xs-12 col-md-4 q-field--with-bottom" filled v-model="tercero.Identificacion"
              type="number" label="Identificacion" lazy-rules />
            <q-select class="col-xs-12 col-md-6 q-field--with-bottom" outlined map-options emit-value
              option-value="IdTipoIdentificacion" option-label="Nombre" v-model="tercero.IdTipoIdentificacion"
              :options="tipoId" label="Tipo" />
            <q-input class="col-xs-12 col-md-2 q-field--with-bottom" outlined v-model="tercero.DV" label="DV" />
            ...
            <q-input class="col-xs-12 col-md-6 q-field--with-bottom" outlined v-model="tercero.Apellido1"
              label="Primer Apellido" lazy-rules />
            ...
            <q-input class="col-xs-12 col-md-12 q-field--with-bottom" outlined v-model="tercero.RazonSocial"
              label="Razon Social" />
            <q-select class="col-xs-12 col-md-4 q-field--with-bottom" outlined filled v-model="tercero.Sexo" map-options
              emit-value option-value="id" option-label="nombre" :options="sexo" label="Sexo">
            ...
            <q-input class="col-xs-12 col-md-4 q-field--with-bottom" outlined stack-label v-model="tercero.Nacimiento"
              type="date" label="Fecha Nacimiento">
              <template v-slot:prepend>
            ...
            <q-select class="col-xs-12 col-md-6 q-field--with-bottom" outlined v-model="tercero.CodigoDepartamento"
              ...
              :options="listaDepartamentos" @input="val => { seleccionarDepartamento(val) }" label="Departamento">
              <template v-slot:prepend>
            ...
            <q-input class="col-xs-12 col-md-3 q-field--with-bottom" outlined type="number" v-model="tercero.Celular"
              label="Celular" />
            <q-input class="col-xs-12 col-md-6 q-field--with-bottom" outlined v-model="tercero.Email"
            ...
            <div class="row justify-center q-pa-sm">
              <q-btn label="Consultar" icon="search" outline align="center" unelevated
                @click.native="buscarTerceroFiltro">
              </q-btn>
            </div>
            <div class="row justify-center q-pa-sm">
              <q-btn label="Limpiar" icon="restore_from_trash" outline align="center" unelevated
            ...
            <q-table class="col-xs-12 col-sm-12 col-md-12" dense flat bordered title="Terceros x Filtro"
              :data="listaPersonas" :columns="columnsTercero" row-key="name" :visible-columns="vcTercero">
              <template v-slot:body-cell-Seleccionar="props">
                <q-td key="Seleccionar" :props="props" auto-width>
                  <q-btn icon="verified" color="black" align="center" outline label=""
                    @click="seleccionarTercero(props.row)" v-close-popup />
                </q-td>
              </template>
              <template v-slot:top="props">
                ...
                <q-space />
                <q-select v-model="vcTercero" multiple outlined dense options-dense
                  :display-value="$q.lang.table.columns" emit-value map-options :options="columnsTercero"
...
export default {
  ...
  methods: {
    ...
    mostrarTipoIdentificacion(idTi, nombreTi) {
      const self = this
      self.$q.loading.show()
      api.get(`/tercero/tipo-id-filtro/${idTi}/${nombreTi}`)
        .then((response) => {
    ...
    mostrarListaDepartamentos() {
      const self = this
      self.$q.loading.show()
      api.get('/usuario/mostrar-departamentos-municipios')
        .then((response) => {
...
export default {
  ...
  methods: {
    ...
    cargarDatosIdentificacion () {
      ...
      self.terceroB = {}
      self.$q.loading.show()
      api.get(`/tercero/mostrar-identificacion/${identificacion}`)
        .then((response) => {
    ...
    buscarPersonaFiltro () {
      ...
      self.listaPersonas = []
      self.$q.loading.show()
      api.post('/tercero/mostrar-filtro', this.tercero)
        .then((response) => {
          self.listaPersonas = response.data
          self.$q.loading.hide()
        })
    ...
    mostrarClientes () {
      ...
      if (self.tercero.Identificacion_persona.length > 0) {
        self.$q.loading.show()
        api.get(`/tercero/mostrar-identificacion/${self.tercero.Identificacion_persona}`)
          .then((response) => {
